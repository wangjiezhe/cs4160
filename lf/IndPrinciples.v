...
Theorem Toy_correct : exists f g,
  forall P : Toy -> Prop,
    ...
    forall t : Toy, P t.
Proof. exists cons1, cons2. exact Toy_ind. Qed.
...
n + (m + p) = (n + m) + p.
Proof.
  intros n m p. generalize dependent n. apply nat_ind.
  - reflexivity.
  - intros n' IHn'. simpl. rewrite IHn'. reflexivity.
Qed.
...
Theorem add_comm''' : forall n m : nat,
  n + m = m + n.
Proof.
  intros n m. generalize dependent n. apply nat_ind.
  - rewrite add_0_r. reflexivity.
  - intros n' IHn'. simpl. rewrite IHn'. apply plus_n_Sm.
Qed.
(* [] *)
...
Proof.
  ...
  - apply ev_0.
  - simpl in H. inversion H.
  - destruct IHn. apply H1. apply H.
  - destruct IHn. apply ev_SS. apply H0. simpl in H. apply H.
Qed.

Lemma even_ev'' : forall n: nat, even n = true -> ev n.
Proof.
  intros. apply even_ev'. apply H.
Qed.
...
Definition better_t_tree_ind : better_t_tree_ind_type :=
  fun X => fun P => fun P_leaf => fun P_branch =>
  fix f (t : t_tree X) :=
    match t with
    | t_leaf => P_leaf
    | t_branch (l, v, r) => P_branch l (f l) v r (f r)
    end.
...
Proof.
  ...
  - reflexivity.
  - intros l IHl v r IHr. simpl. rewrite IHl. rewrite IHr. reflexivity.
Qed.
