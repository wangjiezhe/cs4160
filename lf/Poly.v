...
Fixpoint split {X Y : Type} (l : list (X*Y)) : (list X) * (list Y) :=
  ...
  | (x, y) :: t =>
    ...
    (* let l' := split t in (x :: fst l', y :: snd l') *)
  end.
...
(* Return the total number of characters from a list of strings.
...
Theorem fold_map_correct : forall X Y (f:X -> Y) l,
...
Definition exp (n m : cnat) : cnat :=
  fun (X: Type) (f : X -> X) (x : X) => m (X->X) (fun g i => n X g i) f x.
...
Example exp_4 : exp two three = mult two (mult two (mult two one)).
Proof. reflexivity. Qed.
