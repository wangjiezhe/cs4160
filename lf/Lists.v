...
Fixpoint count (v : nat) (s : bag) : nat :=
  match s with
  | [] => O
  | h :: t => (if v =? h then 1 else 0) + count v t
  end.
...
Theorem bag_count_sum :
  forall v s t, count v (sum s t) = count v s + count v t.
Proof.
  intros v s t.
  induction s as [ | n s'].
  - simpl. reflexivity.
  - simpl.
    rewrite IHs'. rewrite add_assoc. reflexivity.
    (* destruct (v =? n).
...
Proof.
  intros f. intros H. intros n1 n2. intro K.
  rewrite -> H. rewrite <- K. apply H.
